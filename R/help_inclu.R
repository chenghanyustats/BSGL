...
calc_scp <- function(lasso_model, grid_data, ci_level = 0.95) {  # Added ci_level parameter
...
calc_exclusion_map <- function(lasso_model, grid_data) {
  ...
  for (j in 1:m) {
    ...
    exclusion_maps[, j] <- (lower > 0) | (upper < 0)  # Updated exclusion criterion
  }
...
plot_beta_ci_facet <- function(lasso_model, grid_data, beta_indices = 1:3,
                               ...
                               grayscale = FALSE) {  # Added parameter
